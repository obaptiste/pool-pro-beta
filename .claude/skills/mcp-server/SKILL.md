...
For a **successful** result, return the raw structured data plus text via the shared `toolResult(structured, text)` helper in `server.ts`, which wraps them as `{ content: [{ type: 'text', text }], structuredContent: structured }`. Don't hand-roll this shape inline. `text` itself is not always markdown, though: on the 6 tools that take `response_format`, `text` follows it — `JSON.stringify(output, null, 2)` when the caller asked for `'json'`, a markdown-formatting function otherwise — but an *empty*-result branch (no readings/tasks/inventory/equipment found, an empty pagination window) returns the same fixed prose message regardless of `response_format`, and every write tool's success text is always fixed prose too (none of them take `response_format` at all — see above). If you're adding a new success path, only make `text` actually branch on `response_format` when there's real structured data to render two ways; a one-line "nothing found" or "task added" message doesn't need a JSON rendering of itself.
